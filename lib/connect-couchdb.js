...
module.exports = function(connect) {
    ...
    function destroy(docs, fn) {
        ...
        function destroyDocAt(index) {
            if (index === docs.length) {
                ...
                self.destroy(docs[index].id, function(err) {
                    if (err) {
                        console.warn(err.reason + ' [' + docs[index].id + ']');
                    }
    ...
    ConnectCouchDB.prototype.clear = function (fn) {
        this.db.view('_design/connect-sessions/_view/expires', { reduce: false }, function (err, docs) {
            if (err) return fn && fn(err);
    ...
    ConnectCouchDB.prototype.reap = function (fn) {
        var now = +new Date;
        var options = { endkey: now, reduce: false };
    ...
    ConnectCouchDB.prototype.length = function (fn) {
        var now = +new Date;
        var options = { startkey: now, reduce: false };
        this.db.view('_design/connect-sessions/_view/expires', options, function (err, docs) {
            if (err) {
                return fn && fn(err);
            } else {
