...
module.exports = function(connect) {
    ...
    function ConnectCouchDB(opts, fn) {
        ...
        if (env !== 'test') {
            setInterval(this.reap, this.reapInterval);
            setInterval(this.compact, this.compactInterval);
        }
    ...
    ConnectCouchDB.prototype.clear = function (fn) {
        db.view('_design/connect-sessions/_view/all', {include_docs: 'true'}, function (err, docs) {
            if (err) return fn && fn(err);
    ...
    ConnectCouchDB.prototype.reap = function (fn) {
        ...
        db.view('_design/connect-sessions/_view/stale', {include_docs: 'true'}, function (err, docs) {
            if (err) return fn && fn(err);
            var remaining = docs.rows.length;
            docs.rows.forEach(function (doc) {
                this.destroy(doc.value._id, function() {
